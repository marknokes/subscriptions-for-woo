...
class Order
{
     ...
     *
     * @return bool|int the order ID if found, or false if not found
     */
     ...
     * @param null|array $tax_rate_data the tax rate data to be used for calculating taxes
     */
    public static function set_taxes($item, $tax_rate_data = null)
    {
        ...
        if (0 === $tax_rate_data['tax_rate'] || !empty($tax_rate_data['inclusive']) || !$taxes) {
            $item->set_tax_class('');
    ...
    private static function create_line_item($cycle, $total, $sequence, $name)
    {
        ...
        $is_trial_period = 'TRIAL' === $cycle['tenure_type'];
        ...
        if (0 === self::$line_item_price && 'REGULAR' === $cycle['tenure_type']) {
