...
class Plan
{
     ...
     *
     * @param null|string $id The ID of the plan to be created. Defaults to null.
     */
     ...
     * @param null|array $response the response containing the billing cycles
     *
     * @return null|float|string the retrieved value, or null if not found
     */
        ...
        foreach ($billing_cycles as $cycle) {
            if ('REGULAR' === $cycle['tenure_type']) {
                if ('price' === $find) {
                    ...
                    if (isset($cycle['pricing_scheme']['pricing_model'])) {
                        $values = [];
